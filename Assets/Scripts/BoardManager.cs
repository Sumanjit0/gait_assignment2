	...
	
{
	...
	public class BoardManager : MonoBehaviour
	{
		...
		/*public List<Transform> outerWallTilesList;						// For AI Agent
		public List<Transform> innerTilesList;							// For AI Agent
		public List<Transform> foodTilesList;							// For AI Agent
		public List<Transform> enemyTilesList;							// For AI Agent*/
		...
		void BoardSetup ()
		{
			DataHolder.outerWallTilesList = new List<Transform>();
			...
			for (int x = -1; x < columns + 1; x++)
			{
				...
				for(int y = -1; y < rows + 1; y++)
				{
					...
					if(x == -1 || x == columns || y == -1 || y == rows)
                    {
						...
						GameObject outerTile = Instantiate(toInstantiate, new Vector3(x, y, 0f), Quaternion.identity, boardHolder.transform);
						DataHolder.outerWallTilesList.Add(outerTile.transform);
					}
                    else
                    {
						//Instantiate the GameObject instance using the prefab chosen for toInstantiate at the Vector3 corresponding to current grid position in loop, cast it to GameObject.
		...
		void LayoutObjectAtRandom (GameObject[] tileArray, int minimum, int maximum, int tileType)
		{
			...
			for(int i = 0; i < objectCount; i++)
			{
                ...
                GameObject innerTileSpawn = Instantiate(tileChoice, randomPosition, Quaternion.identity, dynamicObjectsHolder.transform);

				if (tileType == 0)
					DataHolder.innerTilesList.Add(innerTileSpawn.transform);
				else if(tileType == 1)
					DataHolder.foodTilesList.Add(innerTileSpawn.transform);
				else
					DataHolder.enemyTilesList.Add(innerTileSpawn.transform);
			}
		}
        ...
        public void SetupScene (int level)
		{
            ...
            LayoutObjectAtRandom(wallTiles, wallCount.minimum, wallCount.maximum, 0); // For Inner Tiles

            //Instantiate a random number of food tiles based on minimum and maximum, at randomized positions.
            LayoutObjectAtRandom(foodTiles, foodCount.minimum, foodCount.maximum, 1); // For Food Tile
        ...
        public void RandomisePlayerAndExitPositions()
        {
			...
			GameObject exitSpawn = Instantiate(exit, exitPos, Quaternion.identity, dynamicObjectsHolder.transform);
			DataHolder.exit = null;
			DataHolder.exit = exitSpawn;
		}
	}
}
